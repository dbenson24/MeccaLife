...
{_x setVariable ["ConvoySM_convoy_enemyDetected", [objNull, []], false]} forEach _convoyUnits;
	...
	for [{_loop = 0},{_loop < count _grpNoTarget},{_loop = _loop + 1}] do {
		...
		if (_group != grpNull) then {
			...
			if (!isNull (_leader findNearestEnemy (getPos _leader))) then {
				...
				{_x setVariable ["ConvoySM_convoy_enemyDetected", [_enemy,_posEnemy], false]} forEach _units;
			};
			if (isNull ((_leader getVariable "ConvoySM_convoy_enemyDetected") select 0) && _d > 20) then {
				_grpNoTarget2 = _grpNoTarget2 - [_group];
			};
...
[_markerArray,_aliveConvoy,_groupsNotNull,_restartConvoy] spawn ConvoySM_Restart;
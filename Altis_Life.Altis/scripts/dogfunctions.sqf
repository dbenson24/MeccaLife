...
if(life_cash < 100000) exitWith {hint"Not enough money bud. Each dog requires lots of care and money";};
...
_actions = 
	...
	while {(_unit getvariable "action")=="true"} do
		...
		if ((_unit getvariable "order") == "idle") then
			{
				_follow = _unit addAction ["<t color = '#ffcc00'>Follow</t>", _dogFollow, [_unit,_playSound]];
				_find = _unit addAction ["<t color = '#ffcc00'>Seek</t>", _dogSeek, [_unit,_playSound]];
				_rest = _unit addAction ["<t color = '#ffff00'>Hide!</t>", _dogHide, [_unit,_playSound]];
				_heel = _unit addAction ["<t color = '#ffcc00'>Heel</t>", _dogHeel, [_unit,_playSound]];
				_unit setvariable ["step","wait"];

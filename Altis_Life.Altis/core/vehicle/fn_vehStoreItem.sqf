...
if(_ctrl == "oil_processed" && {!(EQUAL(life_trunk_vehicle isKindOf "O_Heli_Transport_04_fuel_F"))}) exitWith {hint "You cannot store that in anything but a Taru Fuel!"};

if(_ctrl == "oil_unprocessed" && {!(EQUAL(life_trunk_vehicle isKindOf "O_Heli_Transport_04_fuel_F"))}) exitWith {hint "You cannot store that in anything but a Taru Fuel!"};

...
if({_b = nearestBuilding _x; typeOf _x != "Land_i_Shed_Ind_F"}count life_houses >= (LIFE_SETTINGS(getNumber,"house_limit")) && typeOf(_house) != "Land_i_Shed_Ind_F") exitWith {hint format[localize "STR_House_Max_House",LIFE_SETTINGS(getNumber,"house_limit")]};

if({_b = nearestBuilding _x; typeOf _x == "Land_i_Shed_Ind_F"}count life_houses >= 1 && typeOf(_house) == "Land_i_Shed_Ind_F") exitWith {hint format["You cannot own more than 1 gang warehouse."]};
closeDialog 0;
